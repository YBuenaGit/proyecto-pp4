...
export function buildJuridicalActionContent(parts: Partial<JuridicalActionContentParts>) {
  return [
...
export function parseJuridicalActionContent(content: string | null | undefined): JuridicalActionContentParts {
  const text = clean(content);
  if (!text) return { description: "", guidanceProvided: "", nextStepDescription: "" };
  ...
  if (!matches.length) return { description: text, guidanceProvided: "", nextStepDescription: "" };

  const parts: JuridicalActionContentParts = { description: "", guidanceProvided: "", nextStepDescription: "" };
  matches.forEach((match, index) => {
    ...
    if (match.label === sectionLabels.guidanceProvided) parts.guidanceProvided = value;
    if (match.label === sectionLabels.nextStepDescription) parts.nextStepDescription = value;
  });
