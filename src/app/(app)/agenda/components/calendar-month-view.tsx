...
export function CalendarMonthView({
  ...
  return (
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="grid min-h-14 grid-cols-[1fr_auto_1fr] items-center gap-2 border-b border-slate-200 px-4 py-3">
        <Link
          href={agendaHref(query, { month: previousMonth, day: firstDayOfMonth(previousMonth) })}
          aria-label="Mes anterior"
          className="inline-flex h-9 w-9 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-50 hover:text-slate-950"
        >
          <ChevronLeft className="h-4 w-4" />
        </Link>
        <h2 className="text-center text-base font-semibold capitalize text-[#14213d]">{title}</h2>
      ...
      <div className="w-full">
        <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-50">
          {weekDays.map((day) => (
            <div key={day} className="border-r border-slate-200 px-1.5 py-2 text-center text-[11px] font-semibold text-slate-600 last:border-r-0 sm:px-3 sm:text-xs">
              {day}
            </div>
          ))}
        </div>
