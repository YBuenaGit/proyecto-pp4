...
import { CATALOG_SELECTOR_ITEMS } from "../src/lib/catalog-selector-definitions";

const prisma = new PrismaClient();

const password = "seguridad123";
const destructiveSeedConfirmation = "YES_DELETE_EXISTING_DATA";
...
async function main() {
  if (process.env.ALLOW_DESTRUCTIVE_DEMO_SEED !== destructiveSeedConfirmation) {
    throw new Error(
      `Seed demo bloqueado. Para borrar y recrear los datos demo, define ALLOW_DESTRUCTIVE_DEMO_SEED=${destructiveSeedConfirmation}.`,
    );
  }
  ...
  await prisma.catalogItem.createMany({
    data: CATALOG_SELECTOR_ITEMS.map((item) => ({
      ...item,
      createdById: adminId,
    })),
  });
  ...
  const dispatchSeeds = [
    ...
    [4, "PEDIDO", "BAJA", "DERIVADO", "Solicitud por poda y despeje de camara urbana parcialmente tapada.", "Obras Publicas", byUser.despacho4.id, daysAgo(5, 10)],
